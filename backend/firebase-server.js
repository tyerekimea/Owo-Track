...
app.get("/api/summary", authenticate, async (req, res) => { const all = (await readDocs(expenses(req.user.organization_id))).filter(userScope(req.user)); const countable = all.filter(isBudgetCountable); const totalsByCategory = Object.fromEntries(CATEGORIES.map((category) => [category, countable.filter((e) => e.category === category).reduce((sum, e) => sum + Number(e.amount), 0)])); const month = new Date().toISOString().slice(0, 7); const monthSpentByCategory = Object.fromEntries(CATEGORIES.map((category) => [category, countable.filter((e) => e.category === category && String(e.date).startsWith(month)).reduce((sum, e) => sum + Number(e.amount), 0)])); const budgetRows = await readDocs(budgets(req.user.organization_id)); res.json({ totalsByCategory, grandTotal: countable.reduce((sum, e) => sum + Number(e.amount), 0), count: countable.length, month, monthSpentByCategory, budgets: Object.fromEntries(budgetRows.map((b) => [b.id, b.monthly_limit])) }); });
